...
#define IS_ADD_INTERNAL_CALL(Name) mono_add_internal_call("IS.InternalCalls::" #Name, Name)
    ...
    static void GameSpawnParticleExtraLayer(float x, float y, float direction, float size, float size_scale, float alpha, float alpha_growth, float lifespan, float speed,  MonoString* particle_name, std::tuple<float, float, float> colour, int layer) {
        auto assey_sys = InsightEngine::Instance().GetSystem<AssetManager>("Asset");
        auto system = InsightEngine::Instance().GetSystem<ParticleSystem>("Particle");
        char* c_str = mono_string_to_utf8(particle_name); // Convert Mono string to char*
        std::string part_name(c_str);
    ...
    static void GameSpawnParticleExtraLayerFrames(float x, float y, float direction, float size, float size_scale, float alpha, float alpha_growth, float lifespan, float speed, MonoString* particle_name, std::tuple<float, float, float> colour, int layer ,int col, int row) {
        auto assey_sys = InsightEngine::Instance().GetSystem<AssetManager>("Asset");
        auto system = InsightEngine::Instance().GetSystem<ParticleSystem>("Particle");
        char* c_str = mono_string_to_utf8(particle_name); // Convert Mono string to char*
        std::string part_name(c_str);
    ...
    static void SetButtonIdleAlpha(int entity, float alpha)
    {
        auto& button_component = InsightEngine::Instance().GetComponent<ButtonComponent>(entity);
        button_component.mIdleAlpha = alpha;
    }
    ...
    static void RenderLeftAlignTextFont(MonoString* text, MonoString* font, float x, float y, float size, Vector4D color)
    {
        char* c_str = mono_string_to_utf8(text); // Convert Mono string to char*
        std::string part_name(c_str);
        mono_free(c_str);
        ...
        std::string font_name(f_str);
        mono_free(f_str);
        ISGraphics::mTexts[font_name].addLeftAlignTextRenderCall(part_name, x, y, size, { color.x, color.y, color.z, color.w });
    ...
    void ScriptGlue::RegisterFunctions()
    {
        ...
        IS_ADD_INTERNAL_CALL(SetButtonIdleAlpha);
